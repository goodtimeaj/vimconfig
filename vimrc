...
" Don't show invisible chars (default)
set nolist
...
" Move down and up by display lines instead of real lines
...
nnoremap <silent> <c-n> :tabedit<cr>:NERDTreeFocus<cr>:NERDTreeMirror<cr><c-w>l<cr>

" Quit shortcut for closing tabs, windows, and their buffers
"
" http://vimdoc.sourceforge.net/htmldoc/tabpage.html
" https://vi.stackexchange.com/questions/36384/how-to-count-the-total-number-of-windows-excluding-some-buftype
...
function! CloseView()
  ...
  " Do not close the last tab, ignoring NERDTree
  if win_count == 1 || win_count == 2 && exists('t:NERDTreeBufName') && bufwinnr(t:NERDTreeBufName) != -1 && bufwinnr(t:NERDTreeBufName) != bufwinnr('%')
    if tab_count == 1
      echohl ErrorMsg
      echomsg "Quit to exit last tab"
      echohl NONE
      return
    endif
  endif

  let found_same_buf = v:false
  let current_tab = tabpagenr()
  for tab_idx in range(1, tab_count)
    let tab_bufs = tabpagebuflist(tab_idx)
    let same_bufs = filter(tab_bufs, 'v:val == current_buf')

    if current_tab == tab_idx
      if len(same_bufs) > 1
        let found_same_buf = v:true
      endif
    else
      if len(same_bufs) > 0
        let found_same_buf = v:true
  ...
  " Also quit NERDTree if open
  if win_count == 2 && exists('t:NERDTreeBufName') && bufwinnr(t:NERDTreeBufName) != -1
    :q
  endif
endfunction

nnoremap <silent> <c-w> :call CloseView()<cr>
...
" Toggle NERD tree
nnoremap <silent> <c-\> :call NERDTreeToggleAndRefresh()<cr>
